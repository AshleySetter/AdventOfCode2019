...
int part1(){
  ...
  int wire_1_counter = 0;
  int wire_2_counter = 0;
  for(loc1 = path_wire_1.begin(); loc1 != path_wire_1.end(); loc1++){
	...
	total_steps.push_back(wire_1_counter + wire_2_counter);
	}
      }
    }
  ...
  //  int minElementIndex = std::min_element(total_steps.begin(),total_steps.end()) - total_steps.begin();
  int min_steps = *std::min_element(total_steps.begin(),total_steps.end());
